...
process_peaks <- function(sample){
  ...
  chipnexus <- chipnexus_samples[sample] %>% load_bigwig()
...
summit_list <- cache("processed_chipseq_summit", function(){
  mclapply(names(chipnexus_samples), process_peaks, mc.cores = 3)
})

summit_combined <- lapply(summit_list, function(x){
  gr_r <- resize(x, 201, "center")
  mcols(gr_r) <- NULL
  gr_r}) %>% do.call(c, .) %>% reduce() %>% resize(., 201, "center")

calc_sig <- function(sample, gr){
  nexus <- chipnexus_samples[sample] %>% load_bigwig()
...
sig_df <-  cache("factor_signal_at_combined_summit", function(){
  mclapply(names(chipnexus_samples), function(x)calc_sig(x,summit_combined ), mc.cores = 3) 
})%>% do.call(cbind, .)

mcols(summit_combined) <- sig_df


tss_gr <- get(load("rdata/dme_dm6_fb6.17_tss.gr.RData"))
summit_tss <- queryHits(findOverlaps(query=summit_combined, subject = tss_gr)) %>% unique()
summit_combined <- summit_combined[-1 * summit_tss]

summit_combined$id <- 1:length(summit_combined)
save(summit_combined, file = "rdata/factor_summit_combined_with_signal.RData")
```
...
binding_at_regulatory <- cache("factor_binding_at_enhancer", function(){
  lapply(known_enhancers, function(x){
        overlap_df <- findOverlaps(x, summit_combined)
     if(length(overlap_df) != 0){
        enrich_df <- data.frame(gene = x$gene[queryHits(overlap_df)],
                                gd7_cic = summit_combined$gd7_cic[subjectHits(overlap_df)],
                                tl_cic = summit_combined$tl_cic[subjectHits(overlap_df)],
                                tl_dl = summit_combined$tl_dl[subjectHits(overlap_df)])
     }else{
...
ggplot(cic_df, aes(x = variable, y = gene, fill = value)) + geom_tile() +
  scale_fill_gradient2(high = "firebrick", mid = "peachpuff1", low = "snow",midpoint = 5) +
...
ggplot(dl_df, aes(x = variable, y = gene, fill = value)) + geom_tile() +
  scale_fill_gradient2(high = "dodgerblue4", mid = "slategray1", low = "snow",midpoint = 5) +
  xlab("") + ylab("Enhancer") + ggtitle("factor binding at known enhancers") +
